...
exports.getProfile = async (req, res) => {
  ...
  if (isObject) {
    ...
    if (!attendee) {
      ...
      let profile = await attendeeProfileSchema.findOne({
...
exports.updateInterest = async (req, res) => {
  ...
  if (!isThere) {
    ...
    let resp = await attendeeProfileSchema.updateOne(
      { email: email },
      { $set: { aboutInterest: req.body.aboutInterest } }
    );
...
exports.updateAboutYou = async (req, res) => {
  ...
  if (!isThere) {
    ...
    await attendeeProfileSchema.updateOne(
      { email: email },
      { $set: { aboutYou: req.body._aboutYou } }
    );
...
exports.updateSocialProfile = async (req, res) => {
  ...
  console.log(id);
  if (id === 1) {
    let profile = await attendeeProfileSchema.updateOne(
      { email: req.body.email },
      { facebook: req.body.url }
    );
    console.log(profile);
    res.status(200).send("process successfull");
  }
