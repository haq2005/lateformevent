...
router.post("/updateProfile", async (req, res) => {
  ...
  if (reqId === 2) {
    if (!req.body.newEmail) {
      ...
      await OrganizerRegisterSchema.updateOne(
  ...
  if (reqId === 1) {
    if (!req.body.newFullName) {
      ...
      await OrganizerRegisterSchema.updateOne(
        { email: req.body.email },
        { $set: { fullName: req.body.newFullName } }
      );
  ...
  if (reqId === 3) {
    if (!req.body.newOrg) {
      ...
      await OrganizerRegisterSchema.updateOne(
        { email: req.body.email },
        { $set: { organizationName: req.body.newOrg } }
      );
  ...
  if (reqId === 4) {
    if (!req.body.newPassword) {
      ...
      let getPassword = await organizerRegisterSchema.findOne({
        email: req.body.email,
      });
...
router.post("/updateAttendeeProfile", async (req, res) => {
  if (req.body.id === 1) {
    if (!req.body.newEmail) {
      ...
      await attendeeRegistrationSchema.updateOne(
        { email: req.body.email },
        { $set: { email: req.body.newEmail } }
      );
  ...
  if (req.body.id === 3) {
    if (!req.body.newRecoveryEmail) {
      ...
      await attendeeRegistrationSchema.updateOne(
        { email: req.body.email },
        { $set: { recoveryEmail: req.body.newRecoveryEmail } }
      );
  ...
  if (req.body.id === 4) {
    if (!req.body.newPassword) {
      ...
      let getPassword = await attendeeRegistrationSchema.findOne({
        email: req.body.email,
      });
      console.log(req.body, "c");
      console.log("passsss", getPassword);
      let isSame = await bcrypt.compare(
        req.body.oldPassword,
        getPassword.password
      );
...
router.post("/notificationToken", async (req, res) => {
  let isThere = await attendeeRegistrationSchema.findOne({
    email: req.body.email,
  });
  if (!isThere) {
...
router.post("/organizerNotificationToken", async (req, res) => {
  let isThere = await organizerRegisterSchema.findOne({
    email: req.body.email,
  });
  if (!isThere) {
    res.status(400).send("something went wrong");
