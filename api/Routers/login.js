    ...
    if (err) res.status(400).send("error");
      ...
      if (isValidEmail) {
        ...
        if (isThere) {
          ...
          var otpNumber = otpGenerator.generate(6, {
          ...
          var result = await Otpschema.findOne({
            email: req.body.email,
            role: "login-otp",
          });
          var otp = result.otp + "";
          ...
          transport.sendMail(mailOption, (err, info) => {
            ...
            } else res.status(200).send("email was sent");
          });
        } else {
          var date = new Date();
          var otpNumber = otpGenerator.generate(6, {
          ...
          transport.sendMail(mailOption, (err, info) => {
            if (err) res.status(400).send("some thing went wrong!");
            else res.status(200).send("email was sent");
          });
        }
      } else {
...
router.post("/validateAttendeeOtp", async (req, res) => {
  ...
  if (isEmailValid) {
    ...
    let serverSideOtp = await Otpschema.findOne({
      email: req.body.email,
      role: "login-otp",
    });
    ...
    if (!serverSideOtp) {
      ...
      if (
        clientSideOtp === serverSideOtp.otp &&
        date < serverSideOtp.expiresAt
      ) {
...
router.post("/attendeeEnv", otpValidateAttendeeUser, (req, res) => {
  jwt.verify(req.token, "otp-attendee-lateform", async (err, data) => {
    if (err) res.status(400).send("login first!");
    else {
      console.log(req.body);
      let id = req.body.data.id;
      if (!id) {
        res.status(400).send("something went wrong dooddd");
      } else {
...
router.post("/organizerLogin", async (req, res) => {
  let isRegister = await OrganizerRegisterSchema.findOne({
    email: req.body.email,
  });
  if (!isRegister) {
    res.status(400).send("your account is not register!");
    ...
    if (err) res.status(400).send("error");
      ...
      if (isValidEmail) {
        ...
        if (isThere) {
          ...
          transport.sendMail(mailOption, (err, info) => {
            if (err) res.status(400).send("some thing went wrong!");
            else res.status(200).send("otp was sent to your email");
          });
        } else {
          var date = new Date();
          var otpNumber = otpGenerator.generate(6, {
