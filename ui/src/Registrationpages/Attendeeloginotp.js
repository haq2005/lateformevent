...
function Attendeeloginotp(){
    ...
    let formik = useFormik({
        ...
        onSubmit: data=>{
           ...
           setLoad(true)
            axios.post(process.env.REACT_APP_BACKEND_URL+'/login/validateAttendeeOtp',{
                email:email.email,
                otp:data.otp
            }).then(res=>{
...
<label className="form-label">otp was sent to your registered email</label>
...
{load===false?<button className="btn btn-success btn-full">Verify otp</button>:<button className="btn btn-success btn-full" disabled>verifying...</button>}
</form>
</div>
</div>
        </div>
